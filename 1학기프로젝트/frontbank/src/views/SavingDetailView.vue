  ...
  <div class="background">
    <div class="container mt-5">
      <div class="metadata">
        ...
        <button @click="exitProduct(selectedSaving)" class="btn btn-outline-dark" v-if="isKorCoNmInArray">이미 찜 한 상품입니다 ^^</button>
        <button v-else style="align-content: end;" @click="gaIp( selectedSaving )" class="btn btn-outline-danger">찜하기</button>
      </div>
      <h1 class="text-center mb-4">상품 상세</h1>
      <div class="card p-4">
        <h3 class="mb-3 bank-name">{{ selectedSaving.kor_co_nm }}</h3>
        <h3 class="mb-4 product-name">{{ selectedSaving.fin_prdt_nm }}</h3>
        <p class="month"><strong>공시 제출월:</strong> {{ selectedSaving.dcls_month }}</p>
        <hr>
        <h3 class="money-info">금리 정보</h3><br>
        <table class="table">
          <thead>
            <tr>
              <th class="bold">적립 유형</th>
        ...
        <br>
        <p><strong>만기 후 이자율</strong><br> {{ selectedSaving.mtrt_int }}</p>
        <hr>
        <p><strong>기타 유의사항</strong><br>{{ selectedSaving.etc_note }}</p>
        <p><strong>가입대상:</strong> {{ selectedSaving.join_member }}</p>
        <p><strong>가입방법:</strong> {{ selectedSaving.join_way }}</p>
        <p><strong>최고한도:</strong> {{ formatCurrency(selectedSaving.max_limit) }}</p>
        <p><strong>우대조건:</strong> {{ selectedSaving.spcl_cnd }}</p>
      </div>
    </div>
  </div>
</template>
...
const selectedSaving = computed(() => {
  return savings.value.find(saving => saving.id == id);
})

// 유저가 선택한 상품인지 아닌지 확인하는 변수(T/F 반환)
const isKorCoNmInArray = computed(() => {
  return financialProductsArray.includes(selectedSaving.value?.fin_prdt_nm || '');
})
...
const formatCurrency = (amount) => {
  if (typeof amount === 'number') {
    // 숫자인 경우 숫자를 3자리마다 콤마(,)를 추가하고, 마지막에 통화기호를 추가
...
<style scoped>
/* Flexbox를 사용하여 가로로 나열하도록 스타일 추가 */
...
.interest-info {
  flex: 0 0 auto; /* 각각의 이자 정보가 가로로 나열되도록 설정 */
  margin-right: 10px; /* 각 이자 정보 사이의 간격을 조절할 수 있습니다. */
}
