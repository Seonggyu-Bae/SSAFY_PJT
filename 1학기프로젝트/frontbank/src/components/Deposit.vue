<template>
  ...
  <div class="container mt-5">
    <h1 class="text-center mb-4">예금상품 목록</h1>
    <br>
    ...
    <div class="table-container">
      <table class="table table-striped table-hover">
        ...
        <tbody>
          <tr v-for="deposit in filteredDeposits" :key="deposit.id" @click="goDetail(deposit)">
            <td class="month">{{ deposit.dcls_month }}</td>
            <td class="bankname">{{ deposit.kor_co_nm }}</td>
            <td class="productname">{{ deposit.fin_prdt_nm }}</td>
            <td>
              <div v-if="depositoptions.length">
                <div v-for="option in getSortedOptionInfo(deposit.id)" :key="option.id" class="option-info">
...
const filteredDeposits = computed(() => {
  return deposits.value.length > 0
    ? deposits.value.filter(deposit => deposit.kor_co_nm.includes(bankSearch.value))
    : [];
});
